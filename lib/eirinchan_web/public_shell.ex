defmodule EirinchanWeb.PublicShell do
  ...
  def head_html(active_page, opts \\ []) do
    ...
    <script type="text/javascript">(function(){try{var tz=Intl.DateTimeFormat().resolvedOptions().timeZone;var offset=-new Date().getTimezoneOffset();var current=#{Jason.encode!(browser_timezone)};var currentOffset=#{browser_timezone_offset_minutes};if(!tz&&isNaN(offset)){return;}if(tz===current&&offset===currentOffset){try{sessionStorage.removeItem("eirinchan_timezone_reload");}catch(_e){}return;}if(tz){document.cookie="timezone="+encodeURIComponent(tz)+"; path=/; max-age=31536000; samesite=lax";}if(!isNaN(offset)){document.cookie="timezone_offset="+offset+"; path=/; max-age=31536000; samesite=lax";}try{var reloadKey=(tz||"")+":"+offset;if(sessionStorage.getItem("eirinchan_timezone_reload")!==reloadKey){sessionStorage.setItem("eirinchan_timezone_reload",reloadKey);window.location.reload();}else{sessionStorage.removeItem("eirinchan_timezone_reload");}}catch(_e){window.location.reload();}}catch(_e){}})();</script>
    """
    |> String.trim()
  end
